namespace MapleLib.MapleCryptoLib {
    /// <summary>
    /// Contains all the constant values used for various functions
    /// </summary>
    public static class CryptoConstants {
        /// <summary>
        /// AES UserKey used by MapleStory
        /// </summary>
        public static readonly byte[] UserKey = new byte[] { //16 * 8
            
            };

        /// <summary>
        /// Trimmed AES UserKey used by MapleStory
        /// </summary>
        public static readonly byte[] TrimmedUserKey = new byte[] { //4 * 8
            0x13, 0x00, 0x00, 0x00,
            0x08, 0x00, 0x00, 0x00,
            0x06, 0x00, 0x00, 0x00,
            0xB4, 0x00, 0x00, 0x00,
            0x1B, 0x00, 0x00, 0x00,
            0x0F, 0x00, 0x00, 0x00,
            0x33, 0x00, 0x00, 0x00,
            0x52, 0x00, 0x00, 0x00,
        };

        /// <summary>
        /// ShuffleBytes used my MapleStory to generate a new IV
        /// </summary>
        public static readonly byte[] bShuffle = new byte[] {//16 * 16
            
        };

        /// <summary>
        /// Default AES Key used to generate a new IV
        /// </summary>
        public static byte[] bDefaultAESKeyValue = new byte[] { 0xC6, 0x50, 0x53, 0xF2, 0xA8, 0x42, 0x9D, 0x7F, 0x77, 0x09, 0x1D, 0x26, 0x42, 0x53, 0x88, 0x7C, };

        /// <summary>
        /// IV used to create the WzKey for GMS
        /// </summary>
        public static readonly byte[] WZ_GMSIV = new byte[] { 0x4D, 0x23, 0xC7, 0x2B };

        /// <summary>
        /// IV used to create the WzKey for MSEA
        /// </summary>
        public static readonly byte[] WZ_MSEAIV = new byte[] { 0xB9, 0x7D, 0x63, 0xE9 };

        /// <summary>
        /// Constant used in WZ offset encryption
        /// </summary>
        public const uint WZ_OffsetConstant = 0x581C3F6D;

        /// <summary>
        /// Trims the AES UserKey for use an AES cryptor
        /// </summary>
        public static byte[] createTrimmedUserKey() {
            byte[] key = new byte[32];
            for (int i = 0; i < 128; i += 16) {
                key[i / 4] = UserKey[i];
            }
            return key;
        }
    }
}